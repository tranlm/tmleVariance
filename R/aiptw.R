...
aiptw = function(data, cum.g, Ynodes, Anodes, Cnodes=NULL, abar, Qform, SL.library=NULL, family="quasibinomial", stratify=TRUE) {
	...
	if (!is.matrix(Qform)) {
		...
		for(i in length(Ynodes):1) {
			...
			## IC ##
			IC.all[,i] = calcIC(Q.kplus1=Q.kplus1, Q.k=Qform[,i], h.g.ratio=1/cum.g[,i], uncensored=uncensored, intervention.match=followed.abar)
			Q.kplus1 = Qform[,i]
		}
	}
	
	## Estimating eq corresponding to EIF ##
