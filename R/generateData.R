...
#' @export 
generateData = function(n, time.pt, abar=NULL, beta_pos, beta_ate) {
	...
	QW1     = function(n) pmin(2, pmax(-2, rnorm(n, mean=0, sd=1)))
	QW2     = function(n) rep(plogis(-1), n)
	QW3     = function(n) rnorm(n, mean=0, sd=1)
	QY.t    = function(beta_ate, prev_y, w1, w2, w3, prev_l1, prev_l2, prev_a) ifelse(prev_y==1, 1, plogis(-0.5 + 1.2*w1 - 2.4*w2 - 1.8*prev_l1 - 1.6*prev_l2 + 1*prev_l1*prev_l2 - beta_ate*prev_a))
	QL1.t   = function(y, w1, prev_l1, prev_l2, prev_a) pmin(2, pmax(-2, ifelse(y==1, prev_l1, 0.1 + 0.4*w1 + 0.6*prev_l1 - 0.7*prev_l2 + 0.45*beta_ate*prev_a - rnorm(length(w1), sd=0.5))))
	QL2.t   = function(y, w1, w2, prev_l1, prev_l2, prev_a) pmin(2, pmax(-2, ifelse(y==1, prev_l2, -0.55 + 0.5*w1 + 0.75*w2 + 0.1*prev_l1 + 0.3*prev_l2 + 0.75*beta_ate*prev_a - rnorm(length(w1), sd=0.5))))
	gA.t    = function(beta_pos, y, w1, w2, l1, l2, prev_a) ifelse(y==1, prev_a, ifelse(prev_a==1, 1, plogis(beta_pos - (beta_pos+2.5)*w1 + 1.75*w2 + (beta_pos+3.2)*l1 - 1.8*l2 + 0.8*l1*l2)))
	...
	Qbar.matrix = matrix(ncol=time.pt, nrow=n, dimnames=list(NULL, paste0("Y.", 1:(time.pt))))
	...
	for(i in 0:time.pt){
		...
		if(i==0) {
			...
			#A(t)
			if(is.null(abar)) {
				g.matrix[,"A.0"] = gA.t(beta_pos=beta_pos, y=O[,"Y.0"], w1=O[,"W1"], w2=O[,"W2"], l1=O[,"L1.0"], l2=O[,"L2.0"], prev_a=rep(0, n))
			...
			#Y(t)
			Qbar.matrix[,paste0("Y.",i)] = QY.t(beta_ate=beta_ate, prev_y=O[,paste0("Y.",i-1)], w1=O[,"W1"], w2=O[,"W2"], w3=O[,"W3"], prev_l1=O[,paste0("L1.",i-1)], prev_l2=O[,paste0("L2.",i-1)], prev_a=O[,paste0("A.",i-1)])
			O[,paste0("Y.",i)] = rexpit(Qbar.matrix[,paste0("Y.",i)])
			#L1(t)
			...
			#A(t)
			if(is.null(abar)) {
				g.matrix[,paste0("A.",i)] = gA.t(beta_pos=beta_pos, y=O[,paste0("Y.",i)], w1=O[,"W1"], w2=O[,"W2"], l1=O[,paste0("L1.",i)], l2=O[,paste0("L2.",i)], prev_a=O[,paste0("A.",i-1)])
	...
	return(list(O=O, g.matrix=g.matrix, Qbar.matrix=Qbar.matrix))
